...
expect(_F, "§4 ฐานของตัวที่ดีที่สุด",
       f"max|IC| เฉลี่ย <strong>{_sel['maxABSเฉลี่ยจากความสุ่ม']:.3f}</strong> และถึง <strong>{_sel['maxABSที่95จากความสุ่ม']:.3f}</strong>")
...
expect(_F, "§4 สัดส่วนรอบสุ่มที่สูงกว่า", f"({100 - _sel['เปอร์เซ็นไทล์ของของจริง']:.1f}% ของรอบสุ่มให้ค่าสูงกว่าเรา)")
...
for _k, _v in _samp.items():
    ...
    expect(_F, f"ตาราง §5 · IC {_icv}", f'<td class="nw">{_icv:.2f}</td><td class="nw">{_v:,}</td><td class="nw">{_v/252:.1f}</td>')
expect(_F, "ตาราง §5 · IC ที่วัดได้",
       f'<strong>{abs(_best["IC"]):.3f}</strong> (ที่วัดได้)</td><td class="nw"><strong>{_need:,}</strong></td><td class="nw">{_need/252:.1f}</td>')
...
expect(_F, "ตาราง §6 · หักต้นทุน",
       ...
       + f'<td class="nw"><strong>{_T["เหลือหลังต้นทุนเปอร์เซ็นต์"]:.3f}%</strong>'.replace("-", "−"))
...
expect(_F, "§7 จำนวนตัวอย่างอิสระ",
       f"เหลือเพียง {min(x['จำนวนวันอิสระโดยประมาณ'] for x in _h10)}–{max(x['จำนวนวันอิสระโดยประมาณ'] for x in _h10)}")
...
expect(_F, "ตาราง §8 · IC ที่วัดได้",
       f'<strong>{_lb["IC"]:.3f}</strong> (ที่วัดได้)</td><td class="nw"><strong>{_lb["เดิมพันอิสระที่ต้องใช้ต่อปี"]}</strong></td>')
...
expect(_F, "§8 เดิมพันอิสระที่มีจริง",
       f'{_lb["วันทำการต่อปีของสินทรัพย์เดียว"]} วัน เดิมพัน <em>อิสระ</em> ที่ได้จริงจากสินทรัพย์เดียวมีราว '
       f'<strong>{_lb["เดิมพันอิสระที่มีจริงต่อปีต่อสินทรัพย์"]} ครั้งต่อปี</strong>')
expect(_F, "§8 จำนวนสินทรัพย์ที่ต้องใช้",
       f'ต้องการ {_lb["เดิมพันอิสระที่ต้องใช้ต่อปี"]} แต่มี {_lb["เดิมพันอิสระที่มีจริงต่อปีต่อสินทรัพย์"]} → '
       f'ต้องใช้สินทรัพย์ราว <strong>{_lb["จำนวนสินทรัพย์ที่ต้องใช้"]} ตัว</strong>')
expect(_F, "§8 คริปโตเจ็ดเหรียญ",
       f'คริปโต{_THAI_N.get(_lb["จำนวนสินทรัพย์ที่ต้องใช้"], "เจ็ด")}เหรียญที่ขยับตาม BTC พร้อมกันหมด')
