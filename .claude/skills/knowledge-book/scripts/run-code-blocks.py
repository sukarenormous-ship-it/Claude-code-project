...
for fn in files:
    ...
    for r in blocks:
        ...
        if re.match(r"\s*#\s*──\s*\S+\.py\s*──", code):
            continue
        # บล็อก "ตัวอย่างประกอบ" ที่ยกโค้ดมาชี้ให้ดูว่าผิดตรงไหน — มักมี ...
        # หรืออ้างตัวแปรที่ยังไม่มี · ไม่ได้ตั้งใจให้รัน
        # ติดป้ายด้วยหัว  # ── ตัวอย่างประกอบ ──  (ห้ามใช้พร่ำเพรื่อ:
        # ใช้เฉพาะโค้ดที่ "ยกมาชี้" เท่านั้น ไม่ใช่โค้ดที่รันไม่ผ่านแล้วอยากให้เงียบ)
        if re.match(r"\s*#\s*──\s*ตัวอย่างประกอบ\s*──", code):
            continue
        buf = io.StringIO()
        old = sys.stdout
