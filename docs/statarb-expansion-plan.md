...
| 5 | Generalize ch1–12 (หมายเหตุ F(·) ปิดท้ายทุกบทแกน) + Situation Cards ย้อนหลัง (ch13/14/17/21) + ch0 reading-map table + Appendix A/B เพิ่มสูตร/ศัพท์ใหม่ + ch24 +2 case studies (depeg, FTX vol event) + **กระจาย loss-catalog ครบ 26 ข้อ (A–H1) + บทใหม่ ch19b** | ✅ เขียนแล้ว + ตรวจ render แล้ว | ~50 |
| | **รวม** | | **~175 หน้า** (เล่มโต 406 → ~490+) |

**หมายเหตุ Phase 5:** ทำเสร็จ 7/8 รายการ — **ยังไม่ทำ**: editorial rewrite ของกล่องสูตร ~114 กล่องตาม Formula Style Guide §6 (ของแต่งเติมความสวยงาม ความเสี่ยงต่ำ ประโยชน์ต่ำกว่ารายการอื่น — เสนอข้ามถ้าไม่ร้องขอ) ส่วนการกระจาย loss-catalog (30+ ข้อ ย่อเหลือ 26 ข้อ หลังรวมมุมซ้ำ) **เขียนครบทุกข้อแล้ว** ตาม §สรุปการกระจายลงบท ใน `statarb-loss-catalog.md` — ใหญ่กว่าที่ประเมินไว้ตอนแรกจริง (เพิ่ม ~50 หน้าแทนที่จะเป็น ~25-30) แต่ทำครบตามที่ผู้ใช้ยืนยันแล้ว (รอบ "1" ใน §5 ข้อ 5 ด้านล่าง)
...
5. ✅ **ขอบเขตการกระจาย `statarb-loss-catalog.md`** — ผู้ใช้เลือกตัวเลือก (ก): เขียนครบทั้ง 26 ข้อ (A1-A3, B1-B5, C1-C4, D1-D3, E1-E9, F1-F6, G1, H1) เสร็จสมบูรณ์แล้ว กระจายลงทั้งหมด 13 ไฟล์ (ch3, ch4b, ch5, ch10b, ch11, ch12, ch13c, ch14, ch15, ch17, ch19, ch19b ใหม่, ch20, ch24) — ดูตารางสถานะเต็มใน `statarb-loss-catalog.md` §สรุปการกระจายลงบท
