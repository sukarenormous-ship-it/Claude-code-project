...
## 9. หลักการกำกับ — Connectivity (narrative-first ไม่ใช่ paper-patchwork)

> ⭐ **หลักการสูงสุดของเล่ม:** เนื้อหาขับด้วย **"การเดินทาง + การตัดสินใจของผู้อ่าน"** — paper เป็น *หลักฐานสนับสนุน* (กล่อง `.ba`) ไม่ใช่ *โครงเรื่อง* · ทุกตอนต้องตอบ "ผู้อ่านเอาไปใช้ตัดสินใจอะไรต่อ" และต่อจากตอนก่อนได้

**อุปกรณ์เชื่อมโยงที่จะเพิ่ม (ทำให้ 17 ตอน = เล่มเดียว):**
1. **Through-line เดียว** — เส้นเรื่องใหญ่: *อ่านกระดาน → รู้ใครกำลังกดดัน → ตัดสินใจ execute/วาง quote → คุมความเสี่ยง* (ขยายจาก static→flow→toxicity) + 1 ประโยคธีมต่อองก์ ที่อ้างถึงเส้นนี้
2. **แผนที่ "เราอยู่ตรงไหน" (you-are-here)** — SVG concept map ทั้งเล่ม 1 ภาพ (วางใน Part 0) + mini-map ต้นทุก Part ไฮไลต์จุดปัจจุบันบนเส้นทาง
3. **⭐ Running case เดียวตลอดเล่ม** — ผูกสถานการณ์เดียว (เช่น "จะเข้า long BTC 0.5 ตัวบนเหรียญสภาพคล่องกลาง") ให้ทุก Part หยิบแนวคิดมาใช้กับเคสนี้ทีละชั้น → ผู้อ่านเห็นการต่อยอดเป็นรูปธรรม
4. **Recap + Bridge** ต้น/ท้ายทุก Part — กล่องสั้น "จากตอนก่อนเรารู้ว่า… → ตอนนี้จะเติม… → ส่งต่อตอนหน้า…" (แทนที่ลิงก์ท้ายแบบลอย ๆ)
5. **⭐ บทแคปสโตน (Synthesis)** ปิดเล่ม — decision-workflow/flowchart เดียวที่รวมทั้ง 17 ตอนเป็น "อ่านกระดานจริง 1 รอบ: เปิดจอ → ประเมิน spread/OBI/OFI/VPIN → ตัดสินใจเข้า-ออก/วาง quote → คุม inventory/เสี่ยง" เอาไปใช้ได้ทันที
6. **Paper audit** — ไล่ทุก paper ในเล่ม: ถ้าตอบไม่ได้ว่า "ช่วยผู้อ่านตัดสินใจอะไร" → ย้ายไปภาคผนวก ไม่ยัดในเนื้อหลัก

**ลำดับความคุ้ม:** (5) แคปสโตน + (2) you-are-here map = เปลี่ยนเกมมากสุด ควรทำก่อน · ตามด้วย (3) running case · (4) recap/bridge · (1)(6) เป็นการ refine ทั้งเล่ม
