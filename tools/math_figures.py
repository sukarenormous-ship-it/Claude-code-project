...
expect(_FN, "ข้อมูล · จำนวนวัน", f"BTC {_NT['ข้อมูล']['จำนวนวันผลตอบแทน']} วัน")
...
expect(_FN, "§1 ตลาดขึ้นเท่าไรวันนั้น", f"BTC วิ่งขึ้น {_TL[_naive]['ผลตอบแทนตลาดวันนั้นเปอร์เซ็นต์']:.1f}%")
expect(_FN, "§1 พอร์ตขาดทุนเท่าไร", f"<strong>ขาดทุน {abs(_TL[_naive]['วันแย่ที่สุดเปอร์เซ็นต์']):.1f}%</strong>")
...
expect(_FN, "§4 มิติที่เหลือ", f"เหลือให้สัญญาณเล่นแค่ <strong>{_n_assets - 3} มิติจาก {_n_assets}</strong>")
expect(_FN, "§4 กำไรขั้นต้นลดลงตามขั้น",
       f"{_L[_dol]['กำไรรวมเปอร์เซ็นต์']:.1f}% ที่ขั้น 1 → {_L[_bet]['กำไรรวมเปอร์เซ็นต์']:.1f}% ที่ขั้น 2 "
       f"→ {_L[_fac]['กำไรรวมเปอร์เซ็นต์']:.1f}% ที่ขั้น 3")
...
expect(_FN, "§4 วันแย่สุดของ dollar-neutral",
       f"วันแย่ที่สุดเสีย {_TL[_dol]['วันแย่ที่สุดเปอร์เซ็นต์']:.2f}% ในวันที่ตลาดแทบไม่ขยับเลย "
       f"({_TL[_dol]['ผลตอบแทนตลาดวันนั้นเปอร์เซ็นต์']:.2f}%)".replace("-", "−"))
...
expect(_FN, "§6 ต้นทุนกินเท่าไร",
       ...
       f"จากกำไรขั้นต้น {_F0['กำไรรวมเปอร์เซ็นต์']:.2f}%")
expect(_FN, "§6 IC สัญญาณเร็ว",
       f"เริ่มที่ IC {_D['สัญญาณเร็ว']['1 วัน']:.3f} ในวันแรก")
expect(_FN, "§6 IC สัญญาณเร็ว วันสอง", f"หายไปเกือบหมดในวันที่สอง</strong> ({_D['สัญญาณเร็ว']['2 วัน']:.3f})")
expect(_FN, "§6 IC สัญญาณช้า",
       f"เริ่มต่ำกว่าที่ {_D['สัญญาณช้า']['1 วัน']:.3f} แต่ <strong>ไต่ขึ้นเรื่อย ๆ จนถึง {_D['สัญญาณช้า']['5 วัน']:.3f} ที่ห้าวัน</strong>")
for _lab, _g, _ic in (("เร็ว", _F0, _D["สัญญาณเร็ว"]["1 วัน"]), ("ช้า", _S0, _D["สัญญาณช้า"]["1 วัน"])):
    expect(_FN, f"ตาราง §6 · {_lab} IC", f'<td class="nw">{_ic:.3f}</td>')
    expect(_FN, f"ตาราง §6 · {_lab} turnover/ต้นทุน",
           f'{_g["turnoverเฉลี่ยต่อวัน"]:.3f}</strong></td>' if _lab == "ช้า" else f'{_g["turnoverเฉลี่ยต่อวัน"]:.3f}</td>')
...
expect(_FN, "ตาราง §6 · กำไรสุทธิที่ λ ดีที่สุด (ช้า)",
       f'{_SB["กำไรสุทธิเปอร์เซ็นต์"]:.2f}% (λ = {_SB["ความเร็วปรับพอร์ต"]:g})')
...
expect(_FN, "§6 เท่าตัวของต้นทุน",
       f"จ่ายค่าธรรมเนียมมากกว่า <strong>{_F0['ต้นทุนรวมเปอร์เซ็นต์']/_S0['ต้นทุนรวมเปอร์เซ็นต์']:.1f} เท่า</strong>")
