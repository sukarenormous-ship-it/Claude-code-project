...
## 4.7 มาตรฐานการปรุง v4 — จากคณะ 3 ด้าน (narrative / quant-soul / learning design)

ใช้ทุก Part ต่อจากนี้ + retrofit เล่ม A:
- **🚶 "อ่านเป็นภาษาคนก่อน" เหนือกล่องสูตรทุกกล่อง** (สัญชาตญาณมาก่อนสูตรเสมอ)
- **🧮 worked example + ✍️ ลองทำเอง + 🎯 "3 อย่างที่ต้องจำ"** ต่อการ์ด ★★★ (ปิดช่องว่าง "ใช้เป็น")
- **Signature Line (.pq):** ยก "ประโยคทอง" ที่มีอยู่แล้วในเนื้อออกมาให้เด่น (งานน้อย ผลเยอะ)
- **เล่าเรื่อง:** เปิด section ด้วยปริศนา/เดิมพัน ไม่ใช่ลำดับเหตุการณ์; breath beat คั่นตารางยาว; voice กล้าขึ้น (โทน mentor "พูดตรง ๆ"); ปริศนาข้ามบท + callback; เกร็ดคนจริงกระจายทั่ว
- **สอน *กระบวนการ* judgment ไม่ใช่แค่ผลลัพธ์:** 🎬 "ฉากหน้าจอ" (การตัดสินใจจริง) + 🧭 "นิสัยคิด" (ลำดับคำถามที่ควรถาม) + **heuristic ระดับโต๊ะ** (เช่น Sharpe>2 ให้สงสัยลืมต้นทุน; ถามว่าใครอยู่อีกฝั่ง; neutral ต่ออะไร/ไม่ neutral ต่ออะไร; size ตาม "ทน SD ได้กี่ตัว" ไม่ใช่ความมั่นใจ)
- **ลึกขึ้นใน 🧠:** ดันถึงชั้น "พลิกวิธีมองโลก" ไม่ใช่แค่ "คนทั่วไป vs quant" (เป้าแรก: APT/Prospect/Yield Curve/SDF)
- **ธีม "รักวิชานี้" (💎):** โครงสร้างที่โผล่เอง · นักสืบไม่ใช่นักพยากรณ์ · ผิดอย่างมีประโยชน์ ("all models are wrong, some useful" = วิธีทำงาน) · ถ่อมตัว = ซูเปอร์พาวเวอร์
- **Learning:** reading-path banner (🟢มือใหม่/🔵มีพื้น); glossary จัดชั้น (3 คำต้องจำก่อน); ป้าย [รอบสอง] กล่องลึก; mnemonic; tooltip ห้ามมีศัพท์ที่ยังไม่นิยาม
- **กัน hallucinate:** ยืนยันเกร็ด/ตัวเลขก่อนใช้ (เช่นเกร็ด Savage postcard → ถ้าไม่ยืนยันให้ใช้สำนวนกลาง)
