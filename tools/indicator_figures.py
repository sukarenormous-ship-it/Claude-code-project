...
def build():
    ...
    macd_block = {
        ...
        "ขนาดเทียบข้ามเวลา": {
            ...
            "MACD โตกี่เท่า": r(m_last[1]["macd"] / m_first[1]["macd"], 0),
            "PPO โตกี่เท่า": r((m_last[1]["macd"] / p[idx[m_last[0]]]) / (m_first[1]["macd"] / p[idx[m_first[0]]]), 0),
            "ราคาเปลี่ยนเปอร์เซ็นต์": r(100 * (p[idx[m_last[0]]] / p[idx[m_first[0]]] - 1), 1),
        },
    }
    ...
    bb_block = {
        ...
        "ราคาลบค่าเฉลี่ยวันโฟกัส": r(p[f] - bf["sma"]),
        "ราคาลบค่าเฉลี่ยเป็นกี่SD": r((p[f] - bf["sma"]) / bf["sd"]),
        "ราคาเหนือแบนด์บนเป็นกี่SD": r((p[f] - bf["upper"]) / bf["sd"]),
    }
    ...
    vol_block = {
        ...
        "ตัวอย่างขนาดไม้": {
            "คำอธิบาย": "เสี่ยง 1% ของทุนต่อไม้ · stop = 2 × ATR เมื่อวาน (ตามธรรมเนียม) · ขนาดไม้ = 1% / (stop เป็น % ของราคาเมื่อวาน)",
            "ทุนบาท": 50000,
            "เสี่ยงต่อไม้เปอร์เซ็นต์": 1,
            "stop เป็นเปอร์เซ็นต์ของราคาเมื่อวาน": r(stop_pct, 2),
            "stop เป็นเปอร์เซ็นต์ของราคาวันนี้": r(100 * stop_mult * atr[f - 1] / p[f], 2),
            "ขนาดไม้เป็นเปอร์เซ็นต์ของทุน": r(100 * 1 / stop_pct, 1),
            "ขนาดไม้บาท": int(round(50000 * (1 / stop_pct))),
            "stop จะถูกชนถ้าราคาลง": r(stop_mult * atr[f - 1]),
            "ถ้าวันถัดไปขยับสวนทางเท่าวันนี้": {
                "คำอธิบาย": "ขาดทุนจริง = ขนาดไม้ × (การเคลื่อนไหวเป็นกี่ ATR × ATR เป็น % ของราคา) — stop คือขาดทุนที่วางแผน ไม่ใช่ที่แย่สุด",
                "ขาดทุนเป็นเปอร์เซ็นต์ของทุน": r((1 / stop_pct) * (abs(p[f] - p[f - 1]) / atr[f - 1]) * (100 * atr[f - 1] / p[f - 1]), 2),
            },
        },
    }
